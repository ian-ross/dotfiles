;;; -*- lexical-binding: t -*-
;;; init-helm.el --- The Grand Emacs Incremental Narrowing Thingy.
...
(defun init-helm/gitignore (root files success error)
...
(defun init-helm/files-in-repo (path success error)
  (let ((files (f-files path nil t)))
    (init-helm/gitignore path files
                         (lambda (ignored)
                           (funcall success (-difference files ignored)))
                         error)))

(defun init-helm/find-files-in-emacs-d ()
  (interactive)
  (init-helm/files-in-repo
   dotfiles-dir
   (lambda (files)
     (let ((relfiles (-filter
       ...
       (find-file
        (concat dotfiles-dir
                (helm :sources (helm-build-sync-source ".emacs.d" :candidates relfiles)
                      ...
                      :buffer "*helm emacs.d*")))))
   (lambda (err) (warn "init-helm/find-files-in-emacs-d: %s" err))))

(global-set-key (kbd "C-c C-e") 'init-helm/find-files-in-emacs-d)
